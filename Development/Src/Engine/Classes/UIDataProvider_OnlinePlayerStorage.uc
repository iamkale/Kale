...
event OnRegister(LocalPlayer InPlayer)
{
	...
	if (PlayerControllerId != -1)
	{
		...
		if (OnlineSub != None)
		{
			...
			{
				// Global storage device Id 
				GE = GameEngine(class'Engine'.static.GetEngine());
				DeviceId = GE != None ? GE.GetCurrentDeviceID() : -1;
				...
				if (ReadData(PlayerInterface,PlayerControllerId,DeviceId,Profile) == false)
				{
...
function RefreshStorageData()
{
	...
	if (OnlineSub != None)
	{
		...
		{
			`Log("Login change...requerying storage data",,'DevOnline');
			// Global storage device Id 
			GE = GameEngine(class'Engine'.static.GetEngine());
			DeviceId = GE != None ? GE.GetCurrentDeviceID() : -1;
			// Start the async task
