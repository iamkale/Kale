...
function PeerDesignatedAsHost(name SessionName)
{
	...
	URL = WorldInfo.GetMapName(true) $ "?game=" $ PathName(WorldInfo.GetGameClass()) $ "?listen";
	PeerTravelAsHost(0.5,URL);
}
...
unreliable server function ServerSetSpectatorLocation(vector NewLoc)
{
	...
	{
		`log(GetFuncName() @ Self @ `showvar(GetStateName()) @ `showvar(GetViewTarget()));
